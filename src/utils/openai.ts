...
interface MemberProfile {
    ...
    workoutPreferences: Record<string, any>;
}

interface OpenAIRecommendation {
    trainerId: string;
    personalizedMessage: string;
    workoutPlanSuggestion: string;
...
export const generateEnhancedRecommendations = async (
    ...
    matchedTrainers: TrainerProfile[]
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are a professional fitness consultant AI. Generate personalized trainer recommendations based on member profiles and trainer capabilities. Provide actionable, motivating, and professional advice.',
                    },
                    {
                        role: 'user',
                        content: prompt,
...
const parseOpenAIResponse = (
    ...
    try {
        ...
        if (recommendations.length !== trainers.length) {
            console.warn(
                `Expected ${trainers.length} recommendations, got ${recommendations.length}`
            );
        }
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are an expert fitness trainer creating personalized workout plans. Be specific, practical, and motivating.',
                    },
                    {
                        role: 'user',
                        content: prompt,
                ...
                messages: [
                    {
                        role: 'system',
                        content:
                            'You are a data analyst specializing in fitness program outcomes. Provide actionable insights from training data.',
                    },
                    {
                        role: 'user',
                        content: prompt,
