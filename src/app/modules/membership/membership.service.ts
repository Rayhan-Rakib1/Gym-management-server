...
export class PlanService {
   ...
   */
  static async getAllPlans(query: any) {
    const {
      page = '1',
      limit = '10',
      search,
   ...
   */
  static async createPlan(data: any, createdBy: string) {
    const {
      name,
      description,
      durationDays,
    ...
    if (existingPlan) {
      throw new AppError('Plan with this name already exists', httpStatus.BAD_REQUEST);
    }
  ...
  static async togglePlanStatus(
    ...
    if (!data.isActive) {
      ...
      if (activeMembers > 0) {
        throw new AppError(
          `Cannot deactivate plan. ${activeMembers} active member(s) are using this plan.`,
   ...
   */
  static async deletePlan(planId: string, deletedBy: string) {
    const plan = await prisma.membershipPlan.findUnique({
      where: { id: planId },
      include: {
        _count: {
    ...
    if (plan._count.members > 0) {
      throw new AppError(
        `Cannot delete plan. ${plan._count.members} member(s) are using this plan.`,
        httpStatus.BAD_REQUEST
      );
    }
    ...
    if (plan._count.payments > 0) {
      throw new AppError(
        'Cannot delete plan with existing payment records. Consider deactivating instead.',
        httpStatus.BAD_REQUEST
      );
    }
   ...
   * Get plan members
   */
  static async getPlanMembers(planId: string, page = 1, limit = 10) {
    const plan = await prisma.membershipPlan.findUnique({
      where: { id: planId },
    });
  ...
  static async getPlanStats() {
    ...
    const mostPurchasedPlan = await prisma.membershipPlan.findFirst({
      where: { isActive: true },
      include: {
        _count: {
          select: {
  ...
  static async comparePlans(planIds: string[]) {
    if (planIds.length < 2) {
      throw new AppError('At least 2 plans required for comparison', httpStatus.BAD_REQUEST);
    }

    if (planIds.length > 4) {
      throw new AppError('Maximum 4 plans can be compared at once', httpStatus.BAD_REQUEST);
    }
    ...
    if (plans.length !== planIds.length) {
      throw new AppError('One or more plans not found', httpStatus.NOT_FOUND);
    }
   ...
   */
  static async calculateSavings(planId: string) {
    const plan = await prisma.membershipPlan.findUnique({
      where: { id: planId },
    });
    ...
    const similarPlans = await prisma.membershipPlan.findMany({
      where: {
